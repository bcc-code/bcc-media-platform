...
test("available to", async (t) => {
    let episode = await createEpisodeWith("Test4 Available_to in future", null, faker.date.future())
    ...
    episode = await createEpisodeWith("Test5 Available_to in past", null, faker.date.past())
    ...
    episode = await createEpisodeWith("Test6 Available_to in null", null, null)
    apiResponse = await GetApiResonseforAvailability(episode.id);
