...
func (s *Server) GetPage(c *gin.Context) {
	...
	result, err := s.queries.GetPage(ctx, id)
	if err != nil {
		fmt.Println(err)
		return
	}
...
func (s *Server) GetPages(c *gin.Context) {
	...
	query := goqu.Select().From("admin.page")
	if len(params.Ids) > 0 {
	...
	if col := JsonToDbName(reflect.TypeOf(db.AdminPage{}), sort); col != "" {
		sortCol := goqu.C(col)
		if order == "asc" {
	...
	for _, key := range unhandled {
		if col := JsonToDbName(reflect.TypeOf(db.AdminPage{}), key); col != "" {
...
func (s *Server) CreatePage(c *gin.Context) {
	var params db.InsertPageParams
	if err := c.ShouldBind(&params); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
	...
	ctx := c.Request.Context()
	res, err := s.queries.InsertPage(ctx, params)
	if err != nil {
		fmt.Println(err)
		return
	}
...
func (s *Server) UpdatePage(c *gin.Context) {
	var params db.UpsertPageParams
	if err := c.ShouldBind(&params); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
	ctx := c.Request.Context()
	res, err := s.queries.UpsertPage(ctx, params)
	if err != nil {
		fmt.Println(err)
		return
	}
