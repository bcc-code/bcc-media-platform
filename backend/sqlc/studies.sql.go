...
const isAnswerCorrect = `-- name: IsAnswerCorrect :one
SELECT is_correct FROM questionalternatives qa WHERE id = $1
`

func (q *Queries) IsAnswerCorrect(ctx context.Context, id uuid.UUID) (bool, error) {
	row := q.db.QueryRowContext(ctx, isAnswerCorrect, id)
	var is_correct bool
