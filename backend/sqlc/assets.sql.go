...
WITH audiolang AS (SELECT s.id, array_agg(al.languages_code) langs
     ...
     sublang AS (SELECT s.id, array_agg(al.languages_code) langs
                 ...
                 WHERE e.id = 1
                 GROUP BY s.id)
SELECT 0::int as episodes_id, s.asset_id, s.date_created, s.date_updated, s.encryption_key_id, s.extra_metadata, s.id, s.legacy_videourl_id, s.path, s.service, s.status, s.type, s.url, s.user_created, s.user_updated, al.langs::text[] audio_languages, sl.langs::text[] subtitle_languages
FROM assets a
...
WITH audiolang AS (SELECT s.id, array_agg(al.languages_code) langs
     ...
     sublang AS (SELECT s.id, array_agg(al.languages_code) langs
                 ...
                 WHERE e.id = 1
                 GROUP BY s.id)
SELECT e.id AS episodes_id, s.asset_id, s.date_created, s.date_updated, s.encryption_key_id, s.extra_metadata, s.id, s.legacy_videourl_id, s.path, s.service, s.status, s.type, s.url, s.user_created, s.user_updated, COALESCE(al.langs, array[])::text[] audio_languages, COALESCE(sl.langs, array[])::text[] subtitle_languages
FROM episodes e
         JOIN assets a ON e.asset_id = a.id
         JOIN assetstreams s ON a.id = s.asset_id
         LEFT JOIN audiolang al ON al.id = s.id
