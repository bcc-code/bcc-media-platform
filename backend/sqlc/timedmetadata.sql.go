...
const getTimedMetadataIDsWithRoles = `-- name: getTimedMetadataIDsWithRoles :many
SELECT tm.id
FROM timedmetadata tm
...
type getTimedMetadataIDsWithRolesParams struct {
	Column1 []uuid.UUID `db:"column_1" json:"column1"`
	Column2 []string    `db:"column_2" json:"column2"`
}

func (q *Queries) getTimedMetadataIDsWithRoles(ctx context.Context, arg getTimedMetadataIDsWithRolesParams) ([]uuid.UUID, error) {
	rows, err := q.db.QueryContext(ctx, getTimedMetadataIDsWithRoles, pq.Array(arg.Column1), pq.Array(arg.Column2))
	if err != nil {
		return nil, err
	}
	defer rows.Close()
