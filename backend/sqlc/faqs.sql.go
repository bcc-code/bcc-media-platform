...
const getQuestionsForCategories = `-- name: getQuestionsForCategories :many
WITH t AS (SELECT ts.faqs_id,
                  json_object_agg(ts.languages_code, ts.question) AS question,
                  json_object_agg(ts.languages_code, ts.answer)   AS answer
           FROM faqs_translations ts
...
type getQuestionsForCategoriesRow struct {
	ID       int32                 `db:"id" json:"id"`
	Category int32                 `db:"category" json:"category"`
	Question pqtype.NullRawMessage `db:"question" json:"question"`
	Answer   pqtype.NullRawMessage `db:"answer" json:"answer"`
...
func (q *Queries) getQuestionsForCategories(ctx context.Context, dollar_1 []int32) ([]getQuestionsForCategoriesRow, error) {
	rows, err := q.db.QueryContext(ctx, getQuestionsForCategories, pq.Array(dollar_1))
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []getQuestionsForCategoriesRow
	for rows.Next() {
		var i getQuestionsForCategoriesRow
		if err := rows.Scan(
			&i.ID,
			&i.Category,
			&i.Question,
...
func (q *Queries) listQuestions(ctx context.Context) ([]listQuestionsRow, error) {
