...
// DefaultLanguages is the order of lanuages if none is defined by the user
...
func GetTranslation(langs []string, trans Translations) string {
	langs = append(langs, DefaultLanguages...) // We force the DefaultLangs as the last languages regardless if they have been specified before already
	for _, l := range langs {
