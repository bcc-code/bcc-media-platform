...
module.exports = {
	async up(knex) {
		await knex.raw(episode_access_view_sql);
		await knex.raw(`CREATE UNIQUE INDEX episodes_access_idx ON episodes_access(id);`);
	},

	async down(knex) {
		await knex.raw(`DROP MATERIALIZED VIEW IF EXISTS episodes_access`);
	}
}
