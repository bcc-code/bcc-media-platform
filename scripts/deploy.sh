...
if [ -f "run-route.yaml" ]; then
	...
	gcloud builds submit \
		--project="btv-platform-${ENV}-2" \
		--no-source \
		--substitutions=_API_SERVICE=api-${ENV},_JOBS_SERVICE=background-worker-${ENV},_CMS_SERVICE=directus-${ENV},_REWRITER_SERVICE=rewriter-${ENV},_VIDEOMANIPULATOR_SERVICE=videomanipulator-${ENV},_STREAM_PROXY_SERVICE=stream-proxy-${ENV} \
