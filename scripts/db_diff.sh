...
## RESET THE TEMPDB
psql -h localhost -c "DROP DATABASE IF EXISTS $TEMPDB"
psql -h localhost -c "CREATE DATABASE $TEMPDB"

## RUN EXISTING MIGRATIONS ON TEMP DB
goose postgres "postgres://localhost:${PGPORT}/${TEMPDB}?sslmode=disable" up
...
then
	...
	psql -h localhost -c "INSERT INTO goose_db_version (version_id, is_applied, tstamp) VALUES (${NEW_MIGRATION_NO}, true, NOW());"
else
	echo "Skipping"
fi
