...
gum confirm "Really drop the ${LOCALDB} database? Note that this will disconnect any connected users" && DB_RESET=true || DB_RESET=false
if [[ $DB_RESET == true ]]; then
	gum style --bold "DROP database..."

	# We need to disconnect all users before we can drop the database
	echo "SELECT pg_terminate_backend(pg_stat_activity.pid) FROM pg_stat_activity WHERE pg_stat_activity.datname = '${LOCALDB}' AND pid <> pg_backend_pid();" | psql -U ${PGUSER} -h localhost -d template1
...
for f in $(find ./migrations/special/pre -name \*.sql | sort -V); do
...
gum confirm "Really create the ${LOCALDB} database?" && RUN=true || RUN=false
if [[ $RUN == true ]]; then
	gum style --bold "Create database..."
	echo "CREATE DATABASE ${LOCALDB} WITH OWNER = manager ENCODING = 'UTF8' CONNECTION LIMIT = -1 IS_TEMPLATE = False;" | psql -h localhost -d template1
...
for f in $(find ./migrations/special/post -name \*.sql | sort -V); do
	gum style --foreground 212 --border-foreground 212 --border double --align center --width 100 --margin "1 1" --padding "1 1" "$(head -n1 $f | cut -c 4-)"
	gum confirm "Run ${f}?" && RUN=true || RUN=false
	if [[ $RUN == true ]]; then
