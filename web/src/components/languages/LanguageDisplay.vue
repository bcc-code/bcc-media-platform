...
import LanguageMain from './LanguageMain.vue'
...
onMounted(() => {
  ...
  var audLang = localStorage.getItem('audioLanguage') ?? 'en'
  localStorage.setItem('siteLanguage', audLang)
  var subLang = localStorage.getItem('subtitleLanguage') ?? 'en'
  localStorage.setItem('siteLanguage', subLang)
})
...
<template>
  <Menu as="div" class="relative my-auto">
    {{}}
    <MenuButton :class="open ? '' : 'text-opacity-90'"
