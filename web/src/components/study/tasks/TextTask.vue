    ...
    <div class="p-4 pb-0 flex flex-col items-start justify-start w-full h-full">
        <template v-if="!isDone">
            ...
            <div class="w-full mt-6">
                ...
                <p class="mt-2">Your response will be <span class="font-bold">anonymous</span>.</p>
            </div>
            <div class="flex-1"></div>
            <VButton :disabled="!messageInput" @click="submit" class="w-full" size="large">Send</VButton>
        </template>
    </div>
</template>
...
import { TaskFragment, useCompleteTaskMutation, useSendTaskMessageMutation } from '@/graph/generated';
...
const submit = () => {
    executeMutation({ message: messageInput.value!, taskId: task.value.id }).then(() => {
