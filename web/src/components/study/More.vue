    ...
    <div class="w-full select-none">
        <template v-if="(lesson.studyLesson.tasks.items.length > 0)">
            <div class="p-4 pt-4">
                ...
                <div class="mt-6">
                    <p class="w-full text-white text-style-title-1">{{ t('lesson.watchAgain.title') }}</p>
                    <p class="mt-1 w-full text-white text-style-body-2 text-label-3">
                        {{ t('lesson.watchAgain.description') }}
                    </p>
                    <div class="cursor-pointer mt-4 rounded-xl aspect-video bg-background-2 overflow-hidden relative z-10 relative"
                        @click="playAgain">
                        <div class="bg-gradient-to-b from-[#ffffff00] to-[#000000B3] flex w-full h-full">
                            <img class="object-cover relative -z-10 " :src="episode.image ?? ''" />
                        </div>
                        <div class="absolute top-0 w-full h-full border border-separator-on-light z-40">

                        </div>
                        <div class="absolute top-0 flex items-center justify-center w-full h-full">
                            <VButton size="thin" @click="playAgain" class="z-20">
                                <svg class="inline-block mr-2 -mt-1" width="19" height="21" viewBox="0 0 19 21"
                        ...
                        <div class="absolute top-0 items-end flex w-full h-full" @click="playAgain">
                            <div class="mb-5 ml-4 text-style-title-3">{{ episode.title }}</div>
                        </div>
                    </div>
                </div>
            </div>
