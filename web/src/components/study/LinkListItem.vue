<template>
    <div class="flex items-center justify-start w-full cursor-pointer py-1 border-b border-separator-on-light px-4"
        @click="openLink">
        ...
        <div class="inline-flex flex-col space-y-0.5 items-start justify-start flex-1 pr-4 py-2">
            <p class="w-full text-style-title-3 text-label-1">{{ link.title }}</p>
            <p class="w-full text-style-caption-1 text-label-3">{{ link.description }}</p>
        </div>
