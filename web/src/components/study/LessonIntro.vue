...
const introScreenCode = computed(() => props.lesson.studyLesson.introScreenCode)

onBeforeMount(() => {
    if (!introScreenCode.value) emit('navigate', 'tasks')
})
...
<template>
    <div
        class="p-4 w-full h-full flex flex-col items-center justify-center text-center pb-12 embed:min-h-screen"
    >
        <template v-if="introScreenCode == 'double_your_chances'">
        ...
        <template v-if="introScreenCode == 'quiz_intro'">
            <div class="w-full grow flex flex-col gap-8 mt-8">
                <div class="grow flex flex-col justify-center">
                    <p class="w-full text-white text-style-headline-1 mt-6">
                        {{ t('lesson.intro.quiz') }}
                    </p>
                    <p class="text-style-body-1 text-label-3 mt-1">
                        {{ t('lesson.intro.quizDescription') }}
                    </p>
                </div>
                <div class="rounded-xl bg-separator-on-light p-5 w-full">
                    <p class="text-style-title-2 text-on-tint mb-2">
                        {{ t('lesson.intro.explanationTitle') }}
                    </p>
                    <p class="text-style-body-2 text-label-3 leading-normal">
                        {{ t('lesson.intro.explanation') }}
                    </p>
                </div>
            </div>
        </template>
        <div class="w-full mt-8">
            <p
                v-if="alreadySent"
                class="text-style-caption-1 text-tint-3 mb-4 px-4"
            >
