        ...
        <div class="flex mt-5">
            <div class="flex gap-4" v-if="episode.season">
                <div class="max-w-sm rounded rounded-lg bg-slate-800">
                    ...
                    />
                    <div class="p-4">
                        <h1 class="text-primary text-xl">Episoder ({{episode.season.episodes.total}})</h1>
                        <hr class="border-primary" />
                        <p v-for="e in episode.season.episodes.items" class="cursor-pointer" @click="$router.push({name: 'episode-page', params: {episodeId: e.id}})">
                            {{e.number}}
                            {{e.title}}
                        </p>
                    </div>
