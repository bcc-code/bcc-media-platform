    ...
    <Dialog as="div" @close="emits('closeDialog')" class="relative z-10">
      ...
      <div class="flex min-h-full items-center justify-center p-4 text-center fixed inset-0 overflow-y-auto">
        <TransitionChild as="template" enter-from="-translate-y-full" enter-to="translate-y-0"
          ...
          <DialogPanel
            class="w-full max-w-md transform overflow-hidden rounded-2xl p-6 text-left align-middle shadow-xl transition-all bg-[#1D2838]">
            <DialogTitle class="text-4xl text-center font-bold mb-6">{{ t("settings.profileSetting") }}</DialogTitle>
            <DialogDescription>
              <TabGroup>
                <TabList class="flex space-x-1 rounded-xl p-1 bg-[#707C8E]">
                  <Tab v-for="category in Object.keys(
                    categories
                  )" as="template" :key="category" @click="curretnCategory = category" v-slot="{ selected }">
                    <button :class="[
                      'font-medium w-full rounded-lg py-2.5 text-sm font-medium leading-5 ring-white ring-opacity-60 ring-offset-2 ring-offset-blue-1000 focus:outline-none focus:ring-2 text-blue-700',
                      selected
                        ? 'bg-white shadow'
                ...
                <TabPanels class="mt-2">
                  <TabPanel v-for="(items, idx) in Object.values(
                    categories
                  )" :key="idx" :class="[
  'rounded-xl bg-white p-3 ring-[white] ring-opacity-60 ring-offset-2',
  'ring-offset-blue-400 focus:outline-none focus:ring-2',
]">
                    <LanguageSelector :langlist="items" :selectedtab="curretnCategory"
                      :selectedoption="curretnCategory" />
                  </TabPanel>
                </TabPanels>
              </TabGroup>
            </DialogDescription>
