...
-- Update the materialized view definition to include unlisted episodes in the published column
-- Drop all dependent materialized views in reverse dependency order
...
DROP INDEX IF EXISTS public.episode_availability_id_idx;
DROP MATERIALIZED VIEW public.episode_availability;

-- Recreate episode_availability with unlisted episodes included
...
-- Recreate season_availability (depends on episode_availability)
...
GROUP BY se.id, ea.published, se.available_from, se.available_to, se.show_id, s.status, s.available_from, s.available_to;
...
-- Recreate show_availability (depends on season_availability)
...
-- Recreate filter_dataset (depends on all above)
...
SELECT 'episodes'::text AS collection,
       ...
       COALESCE(mi.agerating_code, e.agerating_code, se.agerating_code, ''::character varying) AS agerating_code,
...
-- Revert the materialized view to the original definition (without unlisted episodes)
-- Drop all dependent materialized views in reverse dependency order
DROP MATERIALIZED VIEW IF EXISTS public.filter_dataset;
DROP MATERIALIZED VIEW IF EXISTS public.show_availability;
DROP MATERIALIZED VIEW IF EXISTS public.season_availability;
...
-- Recreate original episode_availability without unlisted episodes
CREATE MATERIALIZED VIEW public.episode_availability AS
SELECT e.id,
